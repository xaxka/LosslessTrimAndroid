...
> 状态（2026-09-11 恢复）：本文档曾在 1da92e2 随"删除 docs 文档"被误删，
> 但代码注释/单测/verify-timeline.sh 仍持续引用，根因记录断链——现从提交历史
> 原样恢复。文中提到的 `.github/workflows/timeline-regression.yml` 已并入
> `.github/workflows/regression-guard.yml`（单测 + E2E 矩阵合并守卫），其余
> 内容为当时的排查/验证记录，历史原文未改动。
